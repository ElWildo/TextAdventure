...
void section_command(string Cmd, string &wd1, string &wd2) {
  ...
  // I work backwards through the vectors here as a cheat not to invaldate the
  // iterator
  ...
  // Very simple. For the moment I only want the first to words at most (verb /
  // noun).
  if (words.size() == 0) {
  ...
  if (words.size() > 2) {
    cout << "Command too long. Only type one or two words (direction or verb "
            "and noun)"
         << endl;
  }
}
...
void look_around(int loc, room *rms, word *dir) {
  ...
  // LOOK should also allow the player to see what exits exist from the current
  // room.
  for (i = 0; i < DIRS; i++) {
